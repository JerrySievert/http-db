...
function startServer(config) {
  ...
  server.pack.register(require('hapi-auth-jwt-request'), function(err) {
      ...
      validateFunc: function(decoded, request, callback) {
        token.validateToken(decoded, function(err) {
          if (err) {
            ...
            user.authStoreByUsername(store, decoded.username, function(
              err) {
              if (err) {
                callback(null, false);
              } else {
    ...
    server.route({
      ...
      handler: function(request, reply) {
        store.get(encodeURIComponent(request.params.store),
          encodeURIComponent(request.params.id), function(err, data) {
            if (err && err.notFound) {
              reply('').code(204);
            } else {
              reply(data).type('application/json');
    ...
    server.route({
      ...
      handler: function(request, reply) {
        store.keys(encodeURIComponent(request.params.store), function(err,
          data) {
          if (err) {
            reply('').code(204);
          } else {
    ...
    server.route({
      ...
      handler: function(request, reply) {
        store.filter(encodeURIComponent(request.params.store),
          encodeURIComponent(request.query.key), encodeURIComponent(request
            .query
            .value), function(err, stream) {
            if (err) {
              reply('').code(204);
            } else {
    ...
    server.route({
      ...
      handler: function(request, reply) {
        ...
        try {
          ...
          reply('{ "status": "error", "error": "payload should be JSON" }')
            .code(
        ...
        store.put(encodeURIComponent(request.params.store),
          encodeURIComponent(request.params.id), data, function(err) {
    ...
    server.route({
      ...
      handler: function(request, reply) {
        store.query(encodeURIComponent(request.params.store), request.payload,
          function(err, data) {
    ...
    server.route({
      ...
      handler: function(request, reply) {
        store.del(encodeURIComponent(request.params.store),
          encodeURIComponent(request.params.id), function(err) {
    ...
    server.route({
      ...
      handler: function(request, reply) {
        store.all(encodeURIComponent(request.params.store), function(err,
          stream) {
          if (err) {
            reply('').code(204);
          } else {
